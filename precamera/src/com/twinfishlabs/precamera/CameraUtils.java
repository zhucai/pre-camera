...
public class CameraUtils {
    ...
    public static void chooseMaxPreviewSize(Camera.Parameters parms) {
    	int maxWidth = 0;
    	int maxHeight = 0;
        for (Camera.Size size : parms.getSupportedPreviewSizes()) {
        ...
        Log.d(TAG, "Preview size:"+maxWidth+"x"+maxHeight);
        parms.setPreviewSize(maxWidth, maxHeight);
    }

    public static void chooseMaxPictureSize(Camera.Parameters parms) {
    	int maxWidth = 0;
    	int maxHeight = 0;
        ...
        Log.d(TAG, "Picture size:"+maxWidth+"x"+maxHeight);
        parms.setPictureSize(maxWidth, maxHeight);
    }
    ...
    public static int chooseFixedPreviewFps(Camera.Parameters parms, int desiredThousandFps) {
        ...
        if (tmp[0] == tmp[1]) {
        	...
        	if (desiredThousandFps < tmp[0]) guess = tmp[0];
        	else if (desiredThousandFps > tmp[1]) guess = tmp[1];
        	else guess = desiredThousandFps;
//            guess = tmp[0] + (tmp[1] - tmp[0]) / 2;     // shrug
        }
